...
|---|---|---|
| `plans/sablia-site-ux-migration.md` | **Phases 1+2+3+4+5/7 DONE 2026-04-20** — Phase 5 : propagation tokens sable/encre/tuile sur /tarifs, /about, /faq, /cas-clients, /thank-you (0 token legacy restant sur ces pages). Pages légales réécrites via nouveau `LegalShell` + `LegalSection` primitive (CGV art. 293 B CGI, confidentialité n8n self-host + DPF Vercel, mentions légales SIREN/SIRET/APE complet). Legal-prose CSS portée depuis sablia-io (`.legal-section`, `.legal-body`, `.legal-updated`). Docs bumps: INTEGRATIONS.md (WEBHOOK_DIAGNOSTIC prod-confirmed), sitemap.xml (lastmod 2026-04-20 sur legal + gap + roi), SEO.md (section Recent Changes ux-migration). Shims `sablia-*` conservés — LPs et GapForm (out of scope Phase 5) restent en legacy. 27/27 tests verts, tsc clean. Next : Phase 6 validation complète (build + lint + Lighthouse preview). Deadline dure 2026-05-01. | `/execute sablia-site-ux-migration -- Phase 6 validation complète + preview Vercel` |
