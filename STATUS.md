...
|---|---|---|
| `plans/sablia-site-ux-migration.md` | **Phases 1+2/7 DONE 2026-04-20** — Phase 1 tokens/fonts live. Phase 2 primitives livrées : `Reveal` (+ `useReveal` hook), `Button` étendu (variants `primary`/`editorial`/`ghost`/`tuile`) + `ButtonLink`, `AccordionEditorial`, `LogoMark`/`LogoWordmark`, `VoicePlayer`. 27/27 tests verts (12 existants + 15 nouveaux), tsc clean, build OK. Next : Phase 3 sections éditoriales (port 9 sections sablia-io + CalculatorROI conservé, ClientLogosStrip supprimé). Deadline dure 2026-05-01. | `/execute sablia-site-ux-migration -- Phase 3 sections éditoriales` |
